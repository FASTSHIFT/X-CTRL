...
NRF::NRF(SoftwareSPI *spi, NRF_PORT_TYPEDEF nrf_ce)
{
	OccuppiedSPI = spi;
	CE = nrf_ce;
}

bool NRF::DefaultInit(char *addr, int mode)
{
	return Init(addr, NRFDefaultParameters.Channel, NRFDefaultParameters.BaudRate, NRFDefaultParameters.CRCEncodingScheme, NRFDefaultParameters.PacketSize, mode);
}
bool NRF::DefaultInit(char a1, char a2, char a3, char a4, char a5, int mode)
{
	char addr[] = { a1, a2, a3, a4, a5 };
	return Init(addr, NRFDefaultParameters.Channel, NRFDefaultParameters.BaudRate, NRFDefaultParameters.CRCEncodingScheme, NRFDefaultParameters.PacketSize, mode);
}
bool NRF::Init(char *addr, int channel, int baudRateType, int crcEncodingScheme, int packetSize, int mode)
{
	Reset();
	PowerUp();
	SetRXAddr(addr);
	...
	bool success = (r_addr[0] == addr[0]) && (r_addr[1] == addr[1]) && (r_addr[2] == addr[2]) && (r_addr[3] == addr[3]) && (r_addr[4] == addr[4]);
...
bool NRF::Init(char a1, char a2, char a3, char a4, char a5, int channel, int baudRateType, int crcEncodingScheme, int packetSize, int mode)
{
	char addr[] = { a1, a2, a3, a4, a5 };
	return Init(addr, channel, baudRateType, crcEncodingScheme, packetSize, mode);
}
void NRF::FlushTX()
{
