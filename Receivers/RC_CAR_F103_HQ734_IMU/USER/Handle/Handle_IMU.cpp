...
double GyroOffset = 0;
double GyroSum = 0;
volatile int GyroCalTimes = 0;
int TarGyroTimes = 500;
void Task_IMUCalc()
{
    #define RAD(x) (radians(((double)x)*2000.0/32767.0))
    mpu.getMotion6(&MPU_AX, &MPU_AY, &MPU_AZ, &MPU_GX, &MPU_GY, &MPU_GZ);
    double gyroData = RAD(MPU_GX);
    if(GyroCalTimes < TarGyroTimes)
    {
        GyroSum += gyroData;
        GyroCalTimes ++;
    }
    else if(GyroCalTimes == TarGyroTimes)
    {
        GyroCalTimes ++;
        GyroOffset = GyroSum / GyroCalTimes;
    }
    else
    {
        gyroData -= GyroOffset;
