...
public:
    ...
    */
    void      inline writePixel(uint16_t color) {
        SPI_WRITE16(color);
    }
...
protected:
    ...
    int32_t  _cs,            ///< Arduino pin # for chip-select pin
     ...
     RwReg *mosiport,   ///< Direct chip register for toggling MOSI with fast bitbang IO
    ...
    uint16_t  mosipinmask,     ///< bitmask for turning on/off MOSI with fast register bitbang IO
    ...
    uint8_t   invertOnCommand,    ///<  SPI command byte to turn on invert
              invertOffCommand ;           ///<  SPI command byte to turn off invert
    int16_t   _xstart;   ///< Many displays don't have pixels starting at (0,0) of the internal framebuffer, this is the x offset from 0 to align
    int16_t   _ystart;   ///< Many displays don't have pixels starting at (0,0) of the internal framebuffer, this is the y offset from 0 to align
};
