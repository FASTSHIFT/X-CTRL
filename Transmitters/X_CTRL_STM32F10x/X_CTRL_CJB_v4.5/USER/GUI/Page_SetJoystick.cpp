...
{
    int16_t ADC_LX = analogRead_DMA(JSL_X_Pin);
    int16_t ADC_LY = analogRead_DMA(JSL_Y_Pin);
    int16_t ADC_RX = ADC_MaxValue - analogRead_DMA(JSR_X_Pin);
    int16_t ADC_RY = ADC_MaxValue - analogRead_DMA(JSR_Y_Pin);
...
static void Setup()
{
    ...
    for(float i = 0.0; i < 1.0; i += ((1.0 - i) * 0.1 + 0.001))
    {
        ...
        LX_sum += analogRead_DMA(JSL_X_Pin);
        LY_sum += analogRead_DMA(JSL_Y_Pin);
        RX_sum += (ADC_MaxValue - analogRead_DMA(JSR_X_Pin));
        RY_sum += (ADC_MaxValue - analogRead_DMA(JSR_Y_Pin));
        cnt++;
    }
