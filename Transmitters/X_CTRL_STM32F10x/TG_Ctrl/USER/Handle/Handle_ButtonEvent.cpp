...
static void When_ButtonPress()
{
    digitalWrite_HIGH(InfoLED_Pin);
    BuzzTone(500, 20);

    if (btUP == btUP.Press)
        CTRL.Common.Key |= BT_UP;
    if (btDOWN == btDOWN.Press)
        CTRL.Common.Key |= BT_DOWN;
    if (btOK == btOK.Press)
        CTRL.Common.Key |= BT_OK;

    page.PageEventTransmit(EVENT_ButtonPress);
}

static void When_ButtonLongPress()
{
    page.PageEventTransmit(EVENT_ButtonLongPress);
}

static void When_ButtonRelease()
{
