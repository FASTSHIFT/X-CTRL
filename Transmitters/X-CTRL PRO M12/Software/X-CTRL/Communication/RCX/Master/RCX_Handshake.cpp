...
static Pack_t Slave_Grp[RCX_SLAVE_MAX] = {0};//从机握手列表
...
{
    return &(Slave_Grp[index % RCX_SLAVE_MAX]);
}
...
{
    for(int i = 0; i < RCX_SLAVE_MAX; i++)//遍历列表
    {
...
uint8_t Handshake::Process(uint8_t state, uint8_t slaveSelect, CMD_Type cmd)
{
    ...
    case State_Search://广播搜索状态
        ...
        if((Slave_temp.BroadcastHead == BroadcastHead_SlaveWait)//帧头识别码为从机等待连接
                ...
                && (SlaveCnt < RCX_SLAVE_MAX))//未超出从机列表大小
        {
    ...
    case State_ReqConnect://申请连接状态
        SlaveSelectIndex = slaveSelect % RCX_SLAVE_MAX;
