...
{
    HMI_SERIAL.printf("nGear.val=%d", Gear), HMI_end();
}

static void HMI_ShowKph(int spd)
{
    HMI_SERIAL.printf("tSPD.txt=\"%02d\"", spd), HMI_end();
}
...
static void HMI_ShowRPM(int rpm)
{
    ...
    if (Agl != Agl_b)
    {
        HMI_SERIAL.printf("zRPM.val=%d", Agl), HMI_end();
        Agl_b = Agl;
    }
}
...
static void HMI_ShowRPM_Cvs(float rpm, float rpm_cvs)
{
    ...
    HMI_SERIAL.printf("jRPM_Cvs.val=%0.0f", 100.0f * RC), HMI_end();
}

static void HMI_ShowDST(float distance)
{
    HMI_SERIAL.printf("DST.txt=\"%0.2fkm\"", distance / 1000.0f), HMI_end();
}
...
{
    float kph = dfKPH.getNext(xChannel_Passback.Channel_1);
...
void Init_HMI()
{
    ...
    HMI_SERIAL.print("dim=dims");
    HMI_end();
    HMI_SERIAL.print("page DashBoard");
    HMI_end();
    delay(200);
    HMI_Test();
}
...
void Thread_HMI()
{
    ...
    if(HMI_SERIAL.available())
    {
        HMI_SERIAL.readBytes((uint8_t*)&HMI_Data, sizeof(HMI_Data));
