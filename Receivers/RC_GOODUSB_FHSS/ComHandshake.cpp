...
static void ComToMaster(Protocol_Handshake_t *master)
{
  ...
  __IntervalExecute(NRF_ComProcess_Handler(), NRF_ComProcess_TimeMs);
#endif
...
bool HandshakeToMaster(uint8_t setState)
{
  ...
  switch (NowState)
  {
    case HandshakeState_Prepare://握手准备状态
      ...
      HandshakePack_Slave.EnableFunction.Passback = Slave_SupportPassback;//设置是否支持回传
      HandshakePack_Slave.EnableFunction.FHSS = Slave_SupportFHSS;//设置是否支持跳频
      HandshakePack_Slave.Speed = 0;//设置从机通信速度
      HandshakePack_Slave.Commond = CMD_ReqConnect;//设置从机命令:请求连接

#ifdef TIM_Handshake
      TimerSet(TIM_Handshake, NRF_ComProcess_TimeMs * 1000, NRF_ComProcess_Handler); //设置10ms定时中断运行NRF_ComProcess_Handler
    ...
    case HandshakeState_ReqConnect://申请连接状态
      ...
      while (millis() - time < 1000) //连续发送1秒数据包给主机表示可以连接
      {
        ...
        __IntervalExecute(togglePin(LED_Pin), 100);
      }
    ...
    case HandshakeState_Connected: //连接之后状态
      ...
      nrf.SetSpeed(HandshakePack_Attached.Speed);//应用新通信速率
      nrf.SetAutoRetryTimeout(NRF_ComProcess_TimeMs);
      
      if (Slave_SupportPassback)
        State_PassBack = HandshakePack_Attached.EnableFunction.Passback;

      if (Slave_SupportFHSS)
      {
        State_FHSS = HandshakePack_Attached.EnableFunction.FHSS;
        nrfFHSS.SetFreqHoppingList(HandshakePack_Attached.FerqList, FreqList_Length);
      }
