...
template <class T> class DigitalFilter {
public:
    DigitalFilter(uint32_t _filterSamples);
    ~DigitalFilter();
    T getSmooth(T rawIn);
private:
    T *sensSmoothArray;
...
template <class T> DigitalFilter<T>::DigitalFilter(uint32_t _filterSamples)
{
...
template <class T> DigitalFilter<T>::~DigitalFilter()
{
    delete[] sensSmoothArray;
    delete[] sorted;
}

template <class T> T DigitalFilter<T>::getSmooth(T rawIn)
{
    int j, k, top, bottom;
    T total;
    bool done;
