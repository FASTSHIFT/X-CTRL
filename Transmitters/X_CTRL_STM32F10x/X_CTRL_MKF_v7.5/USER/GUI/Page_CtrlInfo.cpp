...
#define PBY_START (screen.height()/2-(PBx_W/2))
...
#define ImageBitmap_X (screen.width()*0.1171875)
#define ImageBitmap_Y (screen.height()*0.234375)

LightGUI::ScrollBar<SCREEN_CLASS> PB_LY(&screen, PBY_START, StatusBar_POS + 7, PBx_W, PBx_L + 12, 1, 4);
LightGUI::ScrollBar<SCREEN_CLASS> PB_RY(&screen, PBY_START, StatusBar_POS + 7, PBx_W, PBx_L + 12, 1, 4);
...
static void Task_DrawProgress()
{
    ...
    PB_LY.setProgress(fmap(CTRL.Left.Y, 0, CtrlOutput_MaxValue, 0.0, 1.01));
...
{
    screen.setCursor(TextMid(0.75, 8), ImageBitmap_Y + 7);
...
static void DisplayCommonInfo()
{
    ...
    {
        if(Common_Passback.Y > StatusBar_POS)
        {
...
static void Setup()
{
    ...
    screen.drawRGBBitmap(ImageBitmap_X, ImageBitmap_Y, gImage_Bitmap_x, 30, 30);
