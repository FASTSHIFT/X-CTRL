...
uint8_t UseCache = 0;
uint8_t Cache_X = 0,Cache_Y = 0;

uint8_t Pos = 0;
uint8_t *bitmaparrayCache[MAX_bitmaparrayCache];

void SSD1306_I2C::init(uint8_t _Address)
{
...
void SSD1306_I2C::setCacheMode(uint8_t chs)
{
	UseCache = chs;
}
...
void SSD1306_I2C::setBrightness(uint8_t Brightness)
{
...
void SSD1306_I2C::setCursor(uint8_t Column, uint8_t Row)
{
	if(UseCache)
	{
...
void SSD1306_I2C::sendData(uint8_t Data)
{
...
void SSD1306_I2C::putChar(uint8_t C)
{
...
void SSD1306_I2C::CachePutChar(uint8_t C)
{
	if(C < 32 || C > 127) //Ignore non-printable ASCII characters. This can be modified for multilingual font.
  {
  	C=' '; //Space
...
void SSD1306_I2C::drawBitmap(uint8_t x, uint8_t y, uint8_t* bitmaparray, uint8_t length)
{
	uint8_t UseCache_BK = UseCache;
...
void SSD1306_I2C::drawBitmap(uint8_t *bitmaparray)
{
	if(UseCache)
	{
...
void SSD1306_I2C::drawFullScreen(const uint8_t *PictureArray,int bytes)
{
	char localAddressMode = addressingMode;
	uint8_t UseCache_BK = UseCache;
...
void SSD1306_I2C::setHorizontalScrollProperties(bool direction,uint8_t startPage, uint8_t endPage, uint8_t scrollSpeed)
{
/*
