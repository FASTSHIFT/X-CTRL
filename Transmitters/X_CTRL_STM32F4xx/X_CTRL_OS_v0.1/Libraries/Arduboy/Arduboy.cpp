...
void Arduboy::eeprom_busy_wait()
{
}

uint8_t Arduboy::eeprom_read_byte(const byte* addr)
{
    return 0;
}

void Arduboy::eeprom_write_byte(uint8_t* addr, uint16_t val)
{
}
...
void Arduboy::fillScreen(uint8_t color)
{
  ...
  for(int16_t i=0; i<sizeof(sBuffer); i++)  { sBuffer[i] = color; }  //sBuffer = (128*64) = 8192/8 = 1024 bytes. 
}
