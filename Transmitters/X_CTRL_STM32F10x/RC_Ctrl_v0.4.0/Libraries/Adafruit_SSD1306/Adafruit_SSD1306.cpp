...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
    ...
    if (sid != -1) {
        pinMode(dc, OUTPUT);
        pinMode(cs, OUTPUT);
#if defined(HAVE_PORTREG) || defined(__STM32__)
...
void Adafruit_SSD1306::ssd1306_command(uint8_t c) {
    if (sid != -1)
    {
        ...
        *csport &= ~cspinmask;
		...
		csport->RegH = cspinmask;
        dcport->RegL = dcpinmask;
        csport->RegL = cspinmask;
#else
...
inline void Adafruit_SSD1306::fastSPIwrite(uint8_t d) {
    ...
    if(hwSPI) {
        ...
        for(uint8_t bit = 0x80; bit; bit >>= 1) {
            ...
            *clkport |=  clkpinmask;
#elif defined(__STM32__)
			clkport->RegL = clkpinmask;
            if(d & bit) mosiport->RegH = mosipinmask;
            else        mosiport->RegL = mosipinmask;
            clkport->RegH = clkpinmask;
#else
