...
static int16_t BaseY = (screen.height() - SCREEN_HEIGHT) / 2;

static void DisplayToScreen(uint8_t *bitmap)
{
...
//            if(bitmap[(row*SCREEN_WIDTH) + (uint8_t)x] &  _BV((uint8_t)y % 8))
//            {
...
void loopIntro()
{
    ...
    //display.display();
    DisplayToScreen(display.getBuffer());
