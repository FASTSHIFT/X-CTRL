/** @Author: _VIFEXTech
  ...
  * @Upgrade:  2019.8.29  - v8.0 文件管理器添加加载提示，添加WAV播放音量控制，在WAV播放器运行时禁用状态栏保证音频不失真 
  * @Upgrade:  2019.8.31  - v8.1 修复WAV播放结束后执行Lua脚本触发的蓝屏BUG，优化Lua强制退出模式
  * @Upgrade:  2019.9.1   - v8.2 将Lua解释器封装为class
  * @Upgrade:  2019.9.3   - v8.3 更新Lua API，修复IMU坐标系Bug，同步游戏与系统的扬声器使能设置 
  */
