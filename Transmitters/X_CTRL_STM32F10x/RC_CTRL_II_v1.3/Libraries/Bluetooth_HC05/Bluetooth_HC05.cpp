...
bool Bluetooth_HC05::AT_Enter()
{
	...
	if(PowerCtrl_Pin != -1)
	{
		if(PowerCtrl_Pin)digitalWrite(PowerCtrl_Pin, LOW);
		if(EN_Pin != -1)digitalWrite(EN_Pin, HIGH);
		delay(1000);
		if(PowerCtrl_Pin)digitalWrite(PowerCtrl_Pin, HIGH);
		delay(1000);
	}
...
void Bluetooth_HC05::AT_Exit()
{
	...
	{
		digitalWrite(PowerCtrl_Pin, LOW);
		if(EN_Pin != -1)digitalWrite(EN_Pin, LOW);
		delay(1500);
		digitalWrite(PowerCtrl_Pin, HIGH);
	}
    Serialx->begin(BaudRate);
