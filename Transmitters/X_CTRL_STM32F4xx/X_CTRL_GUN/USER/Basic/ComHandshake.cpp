...
{
    for(int i = 0; i < SlaveNum_MAX; i++)//遍历列表
    {
...
static void GenerateRandomConfig(Protocol_Handshake_t *hs, uint32_t randSeed)
{
    ...
    {
        hs->FerqList[i] = random(0, 125);//生成随机跳频表 2.4~2.525GHz
    }
}
...
#ifdef TIM_Handshake
        Timer_SetInterrupt(TIM_Handshake, NRF_ComProcess_TimeMs * 1000, NRF_ComProcess_Handler); //设置10ms定时中断运行NRF_ComProcess_Handler
...
//      EEPROM_Handle(EEPROM_Chs::SaveData);//保存所有数据
        ...
        nrf.SetAutoRetryTimeout(NRF_ComProcess_TimeMs - 3);
