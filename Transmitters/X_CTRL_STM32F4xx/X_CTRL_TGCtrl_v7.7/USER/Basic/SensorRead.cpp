...
void Init_Sensors()
{
    ...
    pinMode(ADL_Pin, INPUT_ANALOG_DMA);
    pinMode(ADR_Pin, INPUT_ANALOG_DMA);
...
static void Read_Joystick()
{
    ...
    {
        CTRL.KnobLimit.L = fmap(ADL_ADC(), 0, ADC_MaxValue, 0, CtrlOutput_MaxValue);//左限幅旋钮读取
        CTRL.KnobLimit.R = fmap(ADR_ADC(), 0, ADC_MaxValue, 0, CtrlOutput_MaxValue);//右限幅旋钮读取
    }
    else
    {
        /*限幅置0*/
