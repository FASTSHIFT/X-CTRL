...
/**
  ...
  * @retval 无
  */
void NRF::SetRF_Enable(bool enable)
{
...
float NRF::GetSignalStrength()
{
    ...
    float SignalStrength = (float)TranSuccess_Cnt * 100.0f / Tran_Cnt;
    TranSuccess_Cnt = 0;
    Tran_Cnt = 0;
...
bool NRF::IsDetect()
{
    uint8_t detect = true;
    uint8_t addrBk[5] = {0};
    memcpy(addrBk, ADDRESS, sizeof(addrBk));
    SetAddress(0, 1, 2, 3, 4);
    GetAddress(ADDRESS);
    for(uint8_t i = 0;i < ADR_WIDTH;i++)
    {
        if(ADDRESS[i] != i)
        {
            detect = false;
