...
void pinMode(uint8_t Pin, pinMode_TypeDef pinMode_x)
{
    if(pinMode_x != PWM)
    {
        GPIOx_Init(PIN_MAP[Pin].GPIOx, PIN_MAP[Pin].GPIO_Pin_x, pinMode_x, GPIO_Speed_50MHz);
    }
...
uint16_t analogRead(uint8_t Pin)
{
    if(IS_ADC_PIN(Pin))
    {
        return 0;//Get_ADC(PIN_MAP[Pin].ADCx, PIN_MAP[Pin].ADC_Channel);
    }
    else return 0;
}
...
uint16_t analogRead_DMA(uint8_t Pin)
{
    if(Pin <= PA7)
    {
        return 0;//Get_DMA_ADC(Get_Pinx(Pin));
    }
    else return 0;
}
