...
{
    JSPos_L.setJsPos(
        fmap(JS_L.X, -CtrlOutput_MaxValue, CtrlOutput_MaxValue, -1.0f, 1.0f),
        fmap(JS_L.Y, -CtrlOutput_MaxValue, CtrlOutput_MaxValue, -1.0f, 1.0f)
    );

    JSPos_R.setJsPos(
        fmap(JS_R.X, -CtrlOutput_MaxValue, CtrlOutput_MaxValue, -1.0f, 1.0f),
        fmap(JS_R.Y, -CtrlOutput_MaxValue, CtrlOutput_MaxValue, -1.0f, 1.0f)
    );
}
...
static void Setup()
{
    ...
    float LX_sum = 0.0f, LY_sum = 0.0f, RX_sum = 0.0f, RY_sum = 0.0f;
    uint32_t cnt = 0;
    for(float i = 0.0; i < 1.0f; i += ((1.0f - i) * 0.1f + 0.001f))
    {
