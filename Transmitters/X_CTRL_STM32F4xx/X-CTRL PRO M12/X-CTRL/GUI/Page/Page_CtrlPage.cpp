...
)
{
    ...
    /*摇杆容器*/
    lv_obj_t * contFrame = lv_cont_create(par, NULL);
    static lv_style_t contFrame_style;
    contFrame_style = lv_style_transp;
    contFrame_style.body.radius = 5;
    lv_cont_set_style(contFrame, LV_CONT_STYLE_MAIN, &contFrame_style);
    lv_obj_set_size(contFrame, contJs_Size, contJs_Size);
    lv_obj_align(contFrame, par, align, x_mod, y_mod);

    /*摇杆移动底板*/
    lv_obj_t * contjs = lv_cont_create(contFrame, NULL);
    ...
    lv_cont_set_style(contjs, LV_CONT_STYLE_MAIN, &contjs_style);
    lv_obj_set_size(contjs, 2 * contJs_Size, 2 * contJs_Size);
    lv_obj_align(contjs, NULL, LV_ALIGN_CENTER, 0, 0);
    *contJs = contjs;
    ...
    /*横线*/
    static lv_point_t line1_points[] = { {0, 0}, {2 * contJs_Size, 0}};
    lv_obj_t * line1 = lv_line_create(contjs, NULL);
    lv_line_set_points(line1, line1_points, 2);
    lv_line_set_style(line1, LV_LINE_STYLE_MAIN, &style_line);
    lv_obj_align(line1, NULL, LV_ALIGN_IN_LEFT_MID, 0, 0);

    /*竖线*/
    static lv_point_t line2_points[] = { {0, 0}, {0, 2 * contJs_Size}};
    lv_obj_t * line2 = lv_line_create(contjs, NULL);
    lv_line_set_points(line2, line2_points, 2);
    lv_line_set_style(line2, LV_LINE_STYLE_MAIN, &style_line);
    lv_obj_align(line2, NULL, LV_ALIGN_IN_BOTTOM_MID, 0, 0);

    /*摇杆*/
    lv_obj_t * imgJsStick = lv_img_create(contjs, NULL);
    ...
    lv_obj_align(imgJsFrame, contFrame, LV_ALIGN_CENTER, 0, 0);
}

static void Joystick_Update()
{
    int16_t x = map(CTRL.JS_L.X.Val, -1000, 1000, -contJs_Size, 0);
    int16_t y = -map(CTRL.JS_L.Y.Val, -1000, 1000, 0, contJs_Size);
    lv_obj_set_pos(contJsMove[0], x, y);

    x = map(CTRL.JS_R.X.Val, -1000, 1000, -contJs_Size, 0);
    y = -map(CTRL.JS_R.Y.Val, -1000, 1000, 0, contJs_Size);
    lv_obj_set_pos(contJsMove[1], x, y);
}
...
static void Switch_Event(int event, void* sw)
{
    ...
    for(int i = 0; i < SW_IDX_MAX; i++)
    {
        if(sw == &swGrp[i])
        {
            ...
            else if(event == SwitchEvent_Type::EVENT_SwitchOff)
            {
                state = 1;
            }
...
static void SlaveInfo_NormalUpdate()
{
    if(SlaveInfo_Mode == 0)
    {
        ...
        lv_label_set_text_fmt(
            ...
            "ID: 0x%x  Freq: %dMHz\n"
            "Battery: %0.1fV %d%%",
            RCX::GetTxObjectType(), nrf.RF_RSSI,
            RCX::GetTxObjectID(), nrf.RF_Freq + 2400,
...
static void Setup()
{
    ...
    TabView_Creat();
    Joystick_Creat(&contJsMove[0], tabGrp[0], LV_ALIGN_IN_LEFT_MID, 10, 0);
    Joystick_Creat(&contJsMove[1], tabGrp[0], LV_ALIGN_IN_RIGHT_MID, -10, 0);
...
{
    if(event > SwitchEvent_Type::EVENT_SwitchNone)
    {
