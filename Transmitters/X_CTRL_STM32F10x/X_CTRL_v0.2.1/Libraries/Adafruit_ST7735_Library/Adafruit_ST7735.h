...
#define TFT_SPI SPI_2

#if defined(__STM32F1__)
	#define CS_HIGH() 	//(CS_GPIOx 	-> BSRR = CS_GPIO_Pin_x)
	#define CS_LOW() 		//(CS_GPIOx 	-> BSRR = CS_GPIO_Pin_x)
	#define DC_HIGH()		(DC_GPIOx 	-> BSRR = DC_GPIO_Pin_x)
	#define DC_LOW() 		(DC_GPIOx 	-> BRR 	= DC_GPIO_Pin_x)
	#define SID_HIGH()	(SID_GPIOx	-> BSRR = SID_GPIO_Pin_x)
	#define SID_LOW()		(SID_GPIOx	-> BRR 	= SID_GPIO_Pin_x)
	#define SCLK_HIGH()	(SCLK_GPIOx	-> BSRR = SCLK_GPIO_Pin_x)
	#define SCLK_LOW()	(SCLK_GPIOx	-> BRR 	= SCLK_GPIO_Pin_x)
#elif defined (__KEILDUINO__)
	#define CS_HIGH() 	//digitalWrite_HIGH(_cs)
	#define CS_LOW() 		//digitalWrite_LOW(_cs)
	#define DC_HIGH()		digitalWrite_HIGH(_dc)
	#define DC_LOW() 		digitalWrite_LOW(_dc)
	#define SID_HIGH()	digitalWrite_HIGH(_sid)
	#define SID_LOW()		digitalWrite_LOW(_sid)
	#define SCLK_HIGH()	digitalWrite_HIGH(_sclk)
	#define SCLK_LOW()	digitalWrite_LOW(_sclk)
#else
...
#if defined(__STM32F1__)
	GPIO_TypeDef *CS_GPIOx,*DC_GPIOx,*SID_GPIOx,*SCLK_GPIOx; 
	uint16_t CS_GPIO_Pin_x,DC_GPIO_Pin_x,SID_GPIO_Pin_x,SCLK_GPIO_Pin_x;
#endif
};
