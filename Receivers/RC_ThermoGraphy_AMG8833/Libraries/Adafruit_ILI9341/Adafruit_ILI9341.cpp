 ...
 * */
#ifndef __STM32__
#define GPIO_HIGH(port,mask) (*port|=mask)
#define GPIO_LOW(port,mask)  (*port&=~mask)
#define GPIO_READ(port,mask) ((*port&mask)!=0)
#endif

#ifdef USE_FAST_PINIO
#define SPI_DC_HIGH()           GPIO_HIGH(dcport,dcpinmask)
#define SPI_DC_LOW()            GPIO_LOW(dcport,dcpinmask)
#define SPI_CS_HIGH()           GPIO_HIGH(csport,cspinmask)
#define SPI_CS_LOW()            GPIO_LOW(csport,cspinmask)
#else
...
#ifdef USE_FAST_PINIO
#define SSPI_MOSI_HIGH()        GPIO_HIGH(mosiport,mosipinmask)
#define SSPI_MOSI_LOW()         GPIO_LOW(mosiport,mosipinmask)
#define SSPI_SCK_HIGH()         GPIO_HIGH(clkport,clkpinmask)
#define SSPI_SCK_LOW()          GPIO_LOW(clkport,clkpinmask)
#define SSPI_MISO_READ()        GPIO_READ(misoport,misopinmask)
#else
 ...
 * */
    ...
    #define HSPI_SET_CLOCK() SPI_OBJECT.setClock(100000000);
...
static inline uint8_t _avr_spi_read(void) {
    ...
    return r;
        ...
        #define HSPI_READ()              SPI_OBJECT.transfer(0)//HSPI_WRITE(0)
    #endif
