...
Rcmd2green160x128[] = {              // Init for 7735R, part 2 (mini 160x128)
    2,                        //  2 commands in list:
    ST7735_CASET, 4,          //  1: Column addr set, 4 args, no delay:
    0x00, 2,             //     XSTART = 2
    0x00, 130,             //     XEND = 130
...
void Adafruit_ST7735::commonInit(const uint8_t *cmdList) {
    ...
    if(hwSPI) { // Using hardware TFT_SPI
        TFT_SPI.begin(SPISettings(100000000, MSBFIRST, SPI_MODE0));
#if defined (SPI_HAS_TRANSACTION)
        TFT_SPI.begin();
...
void Adafruit_ST7735::initR(uint8_t options) {
    ...
    if(options == INITR_GREENTAB) {
        ...
        colstart = 2;
        rowstart = 67;
    } else if(options == INITR_MINI160x128) {
        _height = ST7735_TFTHEIGHT_160;
        _width = ST7735_TFTWIDTH_128;
        commandList(Rcmd2green160x128);
        colstart = 2;//xstart
        rowstart = 1;//ystart
    } else {
    ...
    if ((options == INITR_BLACKTAB) || 
        ...
        (options == INITR_MINI128x64) ||
        (options == INITR_MINI160x128)
    )
    {
...
void Adafruit_ST7735::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint16_t color, uint16_t bg) {
    if((x >= _width) || (y >= _height)) return;
    if((x + w - 1) >= _width)  w = _width  - x;
    if((y + h - 1) >= _height) h = _height - y;
...
#define INITR_XXX ((tabcolor == INITR_BLACKTAB) || (tabcolor == INITR_MINI160x80) || (tabcolor == INITR_MINI128x64) ||   (tabcolor == INITR_MINI160x128))
...
void Adafruit_ST7735::drawRGBBitmapFast(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
	if(!hwSPI) return;
	uint32_t len = w * h * 2;
	setAddrWindow(x, y, x + w - 1, y + h - 1);
	CS_LOW();
    DC_HIGH();
    TFT_SPI.send((uint8_t*)bitmap, len);
    CS_HIGH();
}
...
void Adafruit_ST7735::setOpacityX(uint8_t opacity)
{
    SP_Brush_Opacity = opacity;
}
int Adafruit_ST7735::printfX(const char *fmt, ...) {
...
int Adafruit_ST7735::printfX(String str) {
    char buffer[128];
