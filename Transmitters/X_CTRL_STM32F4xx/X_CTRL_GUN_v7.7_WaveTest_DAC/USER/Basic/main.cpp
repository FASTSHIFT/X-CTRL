...
char WaveBuffer123[FIFO_Size];
FifoQueue<char> WaveBuffer(FIFO_Size, WaveBuffer123);

File wavFile;
WAV_TypeDef wav;
void WavePlayer_DataUpdate()
{
    static int counter = 0;
    if(WaveBuffer.available() < 8)
    {
        return;
    }
    else
    {
        ...
        DAC_SetChannel1Data(DAC_Align_12b_R, ((wav.CurrentData.LeftVal + 32768) >> 6) * 0.8f);
    }
}
void BufferUpdate()
{
    static char buffer[4];
    if(WaveBuffer.available() < FIFO_Size - 16)
    {
...
volatile int bufferAvailable = 0;
uint8_t Wav_TestInterface(HWAVEFILE handle, uint8_t size, uint8_t **buffer)
{
    int bufferPos = 0;
    //wavFile.read(BUFFER, size);
    while(size --)
    {
...
void Init_WaveTest()
{
    ...
    TimerSet(TIM2, 1000000 / wav.Header.SampleFreq, WavePlayer_DataUpdate);
    TIM_Cmd(TIM2, ENABLE);
}
