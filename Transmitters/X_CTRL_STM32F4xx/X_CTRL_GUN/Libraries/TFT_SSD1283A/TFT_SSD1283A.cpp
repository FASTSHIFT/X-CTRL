...
void TFT_SSD1283A::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color)
{
    ...
    if((x < 0) || (x >= _width) || (y >= _height) || (h < 1)) return;
    if(y >= 0)
    {
...
{
    // Rudimentary clipping
    if((x >= _width) || (y < 0) || (y >= _height) || (w < 1)) return;
    if(x >= 0)
    {
...
void TFT_SSD1283A::drawFastRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    uint32_t len = w * h;
    setAddrWindow(x, y, x + w - 1, y + h - 1);
    for(uint32_t i = 0; i < len; i++)
    {
        writeData16(bitmap[i]);
    }
}

void TFT_SSD1283A::drawFastBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h)
{
    setAddrWindow(x, y, x + w - 1, y + h - 1);
    ...
    for(int16_t j = 0; j < h; j++)
    {
        for(int16_t i = 0; i < w; i++ )
        {
            ...
            writeData16((b & 0x80) ? White : Black);
        }
    }
}
